...
function BurgerIngredients({ openModal }) {
    ...
    return (<section className={BurgerIngredientsStyles.leftSidebar}>
            <div>
                <div style={{ display: 'flex' }}>
                    {state.tabs.map((el, index) => {
            return (<Tab value={el.name} active={el.active} key={el.id}>
                                {el.name}
                            </Tab>);
        })}
                </div>
            ...
            return (<li key={el._id} onClick={(e) => {
                    openModal(el);
                }}>
                    ...
                    <Counter count={state.addedProducts[el._id]} size="default"/>}
                                        <div className={BurgerIngredientsStyles.listItem}>
                                            ...
                                            <span className={BurgerIngredientsStyles.listItemPrice}>{el.price / 100}
